...
@section('content')
<form method="POST" action="{{ $calculator->exists ? route('admin.calculators.update', $calculator) : route('admin.calculators.store') }}" class="max-w-2xl space-y-4 rounded-xl border bg-white p-6">
    @csrf @if($calculator->exists) @method('PUT') @endif
    <p><label class="text-sm font-medium">Domain</label><select name="source_domain" class="mt-1 block w-full rounded border px-3 py-2">@foreach(['tax','loan'] as $d)<option value="{{ $d }}" @selected(old('source_domain',$calculator->source_domain)===$d)>{{ $d }}</option>@endforeach</select></p>
